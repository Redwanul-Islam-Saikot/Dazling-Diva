    ...
    <div className="space-y-6 max-w-6xl relative">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white border border-slate-100 p-5 rounded-2xl shadow-xs gap-4">
        ...
        <button
          ...
          className="px-4 py-2.5 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all shadow-xs cursor-pointer"
        >
                  ...
                  <tr key={slide._id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center justify-center gap-2">
                        <button type="button" onClick={() => startEdit(slide)} className="p-1.5 hover:bg-amber-50 text-amber-600 rounded-lg transition-all cursor-pointer">
                          <Edit3 className="w-3.5 h-3.5" />
                        </button>
                        <button type="button" onClick={() => handleDelete(slide._id)} className="p-1.5 hover:bg-red-50 text-red-500 rounded-lg transition-all cursor-pointer">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fadeIn">
          <div className="bg-white rounded-2xl shadow-xl border border-slate-100 max-w-4xl w-full max-h-[90vh] overflow-y-auto p-6 relative space-y-6">
            ...
            <button onClick={closeAndResetForm} className="absolute top-4 right-4 p-1.5 hover:bg-slate-100 rounded-xl text-slate-400 hover:text-slate-600 transition-all cursor-pointer">
              <X className="w-5 h-5" />
            </button>
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">Input copy text and high-res dimensions to match front-end client ratios.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <form onSubmit={handleSubmit} className="md:col-span-2 space-y-4">
                ...
                <div>
                  ...
                  <textarea rows={3} value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Ex: Discover Luxury Designer Handloom Sarees..." className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-amber-500 resize-none" />
                </div>
                ...
                <div className="pt-4 flex gap-3 border-t border-slate-100 mt-6">
                  <button
                    ...
                    className="px-5 py-2.5 bg-[#1E1E2D] hover:bg-amber-600 text-white font-bold rounded-xl text-xs flex items-center gap-2 transition-all disabled:opacity-50 shadow-xs cursor-pointer"
                  >
                    {loading ? <><Loader2 className="w-4 h-4 animate-spin" /> Committing Nodes...</> : <><Save className="w-4 h-4" /> Save Configuration</>}
                  </button>
                  <button type="button" onClick={closeAndResetForm} className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl text-xs transition-all cursor-pointer">Cancel</button>
                </div>
              </form>
